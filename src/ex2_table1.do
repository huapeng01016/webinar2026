...
dtable age weight bpsystol i.sex i.race, by(diabetes, nototals tests)  ///
	...
	title(Table 1. Demographics) export(../docs/`export_file'.html, replace)
...
collect export ../docs/`export_file'.docx, replace
collect export ../docs/`export_file'.md, replace
collect export ../docs/`export_file'.pdf, replace
